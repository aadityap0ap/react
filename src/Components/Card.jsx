  ...
  return (
    ...
    {users.map((item,index)=>{
     return <div className='w-52 h-full bg-zinc-100 rounded-lg flex flex-col items-center p-2'>
      ...
      <h1 className='mt-1 text-xl font-semibold text-center'>{item.name}</h1>
      <h4 className='opacity-40 text-xs font-semibold'>{item.email}</h4>
      ...
      <button onClick={()=>handleRemove(id)} className='mt-4 px-3 py-1 bg-red-700 text-xs rounded-md font-semibold text-white'>Remove it</button>
    </div>
    })}
    </>
  )
